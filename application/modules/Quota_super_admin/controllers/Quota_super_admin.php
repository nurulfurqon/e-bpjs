 ...
 *
 */
class Quota_super_admin extends CI_Controller
{

  function __construct()
  {
    parent::__construct();
    $this->load->model('m_quota_super_admin','',TRUE);
  }
  ...
  function index($id=NULL)
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      //pengaturan pagination
			$config["base_url"]=base_url().'Quota_super_admin/index';
			$config["per_page"]=25;
			$config["total_rows"]=$n;
      ...
      {
        $config["per_page"]='';
        $data["tampil"]=$this->m_quota_super_admin->tampil_data($config['per_page'],$id);
      }
      else
      {
        $data["tampil"]=$this->m_quota_super_admin->tampil_data($config['per_page'],$id);
      ...
      $this->load->view('Quota_super_admin/vw_quota_super_admin',$data);
    }
    else
    {
  ...
  function download()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $foto_bukti = $this->m_quota_super_admin->download_data($idnya);
  ...
  function tambah()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      $data['level_jabatan'] = $this->m_quota_super_admin->ambil_level();
      ...
      if ($btn_tambah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          else
          {
            ...
            $simpan = $this->m_quota_super_admin->tambah_data($nik_pegawai,$nama_pengguna,$jenis_kelamin,$ket_pengguna,$ket_dokter,$ket_obat,$tgl_berobat,$ambil_quota,$status_quota,$foto_bukti,$tgl_ambil_quota,$tgl_terima_quota);

            if ($simpan)
            {
              $this->session->set_flashdata('message', 'Data Telah Berhasil Ditambah');
              redirect('Quota_super_admin');
            }
            else
            {
      ...
      $this->load->view('Quota_super_admin/en_quota_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
  ...
  function ambil_jabatan()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $ambil = $this->m_quota_super_admin->getJabatan($level_jabatan);
  ...
  function tes()
  {
    echo $this->load->view('Quota_super_admin/test');
  }
  ...
  function ambil_quota()
  {
    ...
    {
      $nik_pegawai = $this->input->post('nik_pegawai');
      $ambil['ambil'] = $this->m_quota_super_admin->getQuota($nik_pegawai);
      if ($nik_pegawai == 0) {
        $data = $this->load->view('Quota_super_admin/ajaxRange2');
        echo $data;
      }
      else
      {
        $data = $this->load->view('Quota_super_admin/ajaxRange',$ambil);
        echo $data;
      }
    }
    else
  ...
  function detail()
  {
    ...
    {
      $idnya = $this->uri->segment(3);
      $data['detail'] = $this->m_quota_super_admin->detail_data($idnya);
      ...
      if ($btn_ubah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          $ubah = $this->m_quota_super_admin->ubah_data($idnya,$nama_pengguna,$jenis_kelamin,$ket_pengguna,$ket_dokter,$ket_obat,$tgl_berobat,$status_quota,$tgl_terima_quota);
          if($ubah)
          {
      ...
      $this->load->view('Quota_super_admin/dt_quota_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
    ...
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
     ...
     if (!empty($_FILES)) {
       ...
       else
       {
         ...
         $ubah_gambar = $this->m_quota_super_admin->ubah_foto($idnya,$foto);
  ...
  function hapus()
  {
    ...
    {
      $idnya = $this->uri->segment(3);
      $this->m_quota_super_admin->hapus_data($idnya);
