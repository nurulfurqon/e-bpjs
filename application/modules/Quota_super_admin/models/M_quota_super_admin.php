 ...
 *
 */
class M_quota_super_admin extends CI_Model
{
  ...
  function tampil_data($perpage,$uri)
  {
    ...
    if (empty($cari_tanggal) && empty($cari_nik))
    {
      ...
      $this->db->where("b.nik = a.nik_pegawai");
    ...
    else
    {
      $this->db->select("a.*,b.nama");
      $this->db->from("tb_ambil_quota a,tb_pegawai b");
      $this->db->where("b.nik = a.nik_pegawai AND a.tgl_ambil_quota LIKE '$cari_tanggal%' AND a.nik_pegawai LIKE '%$cari_nik%'");
      $this->db->order_by("a.tgl_ambil_quota","DESC");
    }
  ...
  function ubah_data($idnya,$nama_pengguna,$jenis_kelamin,$ket_pengguna,$ket_dokter,$ket_obat,$tgl_berobat,$status_quota,$tgl_terima_quota)
  {
    ...
    else
    {
      $ubah = $this->db->query("UPDATE tb_ambil_quota SET
        ...
        tgl_terima_quota = '$tgl_terima_quota'
