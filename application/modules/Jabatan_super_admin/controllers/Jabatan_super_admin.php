 ...
 *
 */
class Jabatan_super_admin extends CI_Controller
{

  function __construct()
  {
    parent::__construct();
    $this->load->model('m_jabatan_super_admin','',TRUE);
  }
  ...
  function index()
  {
    ...
    {
      $data['tampil_data'] = $this->m_jabatan_super_admin->tampil_data();
      $this->load->view('Jabatan_super_admin/vw_jabatan_super_admin',$data);
    }
    else
    {
    ...
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      if ($btn_tambah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          /* end */
          $simpan = $this->m_jabatan_super_admin->tambah_data($level_jabatan,$nama_jabatan,$quota);
          if ($simpan)
          {
            $this->session->set_flashdata('message', 'Data Telah Berhasil Ditambah');
            redirect('Jabatan_super_admin');
          }
          else
          {
      ...
      $this->load->view('Jabatan_super_admin/en_jabatan_super_admin',$pesan);
    }
    else
    {
      redirect('Login_super_admin');
  ...
  function detail()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $data['detail'] = $this->m_jabatan_super_admin->detail_data($idnya);
      ...
      if ($btn_ubah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          $quota = str_replace($satu,$dua,$quota);
          /* end */
          $ubah = $this->m_jabatan_super_admin->ubah_data($idnya,$nama_jabatan,$quota);
          if($ubah)
          {
      ...
      $this->load->view('Jabatan_super_admin/dt_jabatan_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
  ...
  function hapus()
  {
    ...
    {
      $idnya = $this->uri->segment(3);
      $this->m_jabatan_super_admin->hapus_data($idnya);
