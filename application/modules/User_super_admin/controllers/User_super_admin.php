...
class User_super_admin extends CI_Controller
{
  ...
  {
    parent::__construct();
    $this->load->model('m_user_super_admin','',TRUE);
  }
  ...
  function index($id=NULL)
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      //pengaturan pagination
			$config["base_url"]=base_url().'User_super_admin/index';
			$config["per_page"]=25;
			$config["total_rows"]=$n;
      ...
      {
        $config["per_page"]='';
        $data["tampil"]=$this->m_user_super_admin->tampil_data($config['per_page'],$id);
      }
      else
      {
        $data["tampil"]=$this->m_user_super_admin->tampil_data($config['per_page'],$id);
      ...
      $this->load->view('User_super_admin/vw_user_super_admin',$data);
    }
    else
    {
  ...
  function tambah()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $data['error'] = "";
      $data['level_jabatan'] = $this->m_user_super_admin->ambil_level();
      if ($btn_tambah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          $simpan = $this->m_user_super_admin->tambah_data($nik_pegawai,$password,$status);
          if ($simpan)
          {
            $this->session->set_flashdata('message', 'Data Telah Berhasil Ditambah');
            redirect('User_super_admin');
          }
          else
          {
      ...
      $this->load->view('User_super_admin/en_user_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
  ...
  function detail()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $data['detail'] = $this->m_user_super_admin->detail_data($idnya);
      ...
      if ($btn_ubah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          $ubah = $this->m_user_super_admin->ubah_data($idnya,$password,$status);
          if($ubah)
          {
      ...
      $this->load->view('User_super_admin/dt_user_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
  ...
  function ambil_jabatan()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $ambil = $this->m_user_super_admin->getJabatan($level_jabatan);
  ...
  function hapus()
  {
    ...
    {
      $idnya = $this->uri->segment(3);
      $this->m_user_super_admin->hapus_data($idnya);
