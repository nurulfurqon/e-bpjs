 ...
 *
 */
class Pegawai_super_admin extends CI_Controller
{

  function __construct()
  {
    parent::__construct();
    $this->load->model('m_pegawai_super_admin','',TRUE);
  }
  ...
  function index()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      //pengaturan pagination
			$config["base_url"]=base_url().'Pegawai_super_admin/index';
			$config["per_page"]=25;
			$config["total_rows"]=$n;
      ...
      if (!empty($cari_jabatan)||!empty($cari_nik)) {
        $config["per_page"]='';
        $data["tampil"]=$this->m_pegawai_super_admin->tampil_data($config['per_page'],$id);
      }
      else {
        $data["tampil"]=$this->m_pegawai_super_admin->tampil_data($config['per_page'],$id);
      }
			$data["no"]=$id;
      $data["jabatan"] = $this->m_pegawai_super_admin->ambil_level();
      ...
      $this->load->view('Pegawai_super_admin/vw_pegawai_super_admin',$data);
    }
    else
    {
  ...
  function update_quota()
  {
    ...
    if ($bulan == 12)
    {
      ...
      $update = $this->m_pegawai_super_admin->update_quota($tgl_update);
      if ($update)
      {
        $this->session->set_flashdata('message', 'Quota Telah Diperbarui');
        redirect('Pegawai_super_admin');
      }
    }
    else
    {
  ...
  function tambah()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $data['level_jabatan'] = $this->m_pegawai_super_admin->ambil_level();
      ...
      if ($btn_tambah)
      {
        ...
        if ($this->form_validation->run()==TRUE)
        {
          ...
          else
          {
            ...
            $simpan = $this->m_pegawai_super_admin->tambah_data($nik,$nama,$alamat,$tempat_lahir,$tgl_lahir,$jenis_kelamin,$telepon,$status,$foto,$level_pegawai,$tgl_mulai_kerja,$no_bpjs,$tgl_update);
      ...
      $this->load->view('Pegawai_super_admin/en_pegawai_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
  ...
  function detail()
  {
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
      ...
      $data['detail'] = $this->m_pegawai_super_admin->detail_data($idnya);
      $data['level_jabatan'] = $this->m_pegawai_super_admin->ambil_level();
      $nama = $this->input->post('nama');
      $alamat = $this->input->post('alamat');
      $tempat_lahir = $this->input->post('tempat_lahir');
      ...
      if ($btn_ubah)
      {
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          $tgl_lahir = $tgl_l[2].'-'.$tgl_l[1].'-'.$tgl_l[0];
          /* end */
          $ubah = $this->m_pegawai_super_admin->ubah_data($idnya,$nama,$alamat,$tempat_lahir,$tgl_lahir,$jenis_kelamin,$telepon,$status,$level_pegawai,$tgl_mulai_kerja,$no_bpjs);
          if($ubah)
          {
      ...
      $this->load->view('Pegawai_super_admin/dt_pegawai_super_admin',$data);
    }
    else
    {
      redirect('Login_super_admin');
    ...
    if ($this->session->userdata('ses_s_adm_id') == TRUE)
    {
     ...
     if (!empty($_FILES)) {
       ...
       else
       {
         $uploaded = $this->upload->data();//untuk mengambil data upload
         $foto = $uploaded['file_name'];//data upload yang pertama
         $ubah_gambar = $this->m_pegawai_super_admin->ubah_foto($idnya,$foto);
  ...
  function hapus()
  {
    ...
    {
      $idnya = $this->uri->segment(3);
      $this->m_pegawai_super_admin->hapus_data($idnya);
